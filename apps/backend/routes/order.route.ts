...
orderRoute.post("/marketOrder",(req,res)=>{
    // eg YES , BUY , 10
    let {userId,ticket_type,order_type,quantity}:MarketOrderRequest=req.body;
    ...
    try {
        ...
        if (counterSide.totalPriceQty <quantity) {
          ...
          for(let i=0;i<counterSide.priceLevels.length && remainingQuantity>0;i++){
            ...
            const startingPriceLevelQuantity = priceLevel.totalQty;
...
orderRoute.post("/limitOrder",(req,res)=>{
    try {
        ...
        const {order_type,ticket_type,quantity,price}:LimitOrderRequest=req.body
    ...
    if (remainingQuantity > 0) {
        ...
        let priceLevel = orderSide.priceLevels.find(level => level.price === price);
