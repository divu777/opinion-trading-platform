// Enhanced Opinion Trading Manager with Fixes
...
type Market_Info = Record<string, {
    title?: string;
    description?: string;
    YES?: number ;
    NO?: number ;
...
export class Manager {
    ...
    printOrderBook() {
        ...
        console.log("Stock Balances:", JSON.stringify(this.Stock_Balances, null, 2));
    }
    ...
    private distributeWinnings(marketId: string, winner: "YES" | "NO") {
        for (const userId in this.Stock_Balances) {
            ...
            if (userStocks && userStocks[winner]) {
                ...
                console.log(`Paid ${payout/100} to user ${userId} for ${winningShares} winning shares`);
            }
        }
                ...
                for (const priceLevel in bookSide.priceLevels) {
                    ...
                    for (const order of level.orders) {
                        if (order.userId !== "ADMIN" && order.userId !== "SYSTEM") {
                            ...
                            if (orderType === "BUY") {
                                const refund = parseFloat(priceLevel) * order.stock_quantity * 100;
    ...
    getAllMarkets(data: Extract<SubscribeMessageType, { type: "GET_ALL_MARKETS" }>) {
        const markets = Object.keys(this.OrderBook).filter(marketId => 
            !this.Market_Info[marketId]?.isResolved
        );

        return {
            eventId: data.eventId,
            payload: {
                message: markets.length > 0 ? "Active markets found" : "No active markets",
                markets,
    ...
    async handleBuyOrder(data: Extract<SubscribeMessageType, { type: "BUY" }>) {
        ...
        for (const priceLevel of priceKeys) {
            if (priceLevel <= price && remainingQuantity > 0) {
                ...
                while (levelData.orders.length > 0 && remainingQuantity > 0) {
                    ...
                    const matchQty = Math.min(sellOrder.stock_quantity, remainingQuantity);
                    
                    if (sellOrder.userId !== "ADMIN" && sellOrder.userId !== "SYSTEM") {
                        const sellerStock = this.Stock_Balances[sellOrder.userId]?.[marketId]?.[ticket_type] || 0;
                        if (sellerStock < matchQty) {
                            console.error(`Seller ${sellOrder.userId} doesn't have enough stock`);
                    ...
                    // Execute the trade
                    this.executeTrade(userId, sellOrder.userId, marketId, ticket_type, matchQty, priceLevel);
        ...
        if (remainingQuantity > 0) {
            this.createOppositeOrder(marketId, ticket_type, price, remainingQuantity, userId);
            this.placeOrderOnBook(marketId, ticket_type, price, remainingQuantity, userId,"BUY");
    ...
    async handleSellOrder(data: Extract<SubscribeMessageType, { type: "SELL" }>) {
        ...
        const userStock = this.Stock_Balances[userId]?.[marketId]?.[ticket_type] || 0;
        ...
        for (const priceLevel of priceKeys) {
            if (priceLevel >= price && remainingQuantity > 0) {
                ...
                while (levelData.orders.length > 0 && remainingQuantity > 0) {
                    ...
                    this.executeTrade(buyOrder.userId, userId, marketId, ticket_type, matchQty, priceLevel);
        ...
        if (remainingQuantity > 0) {
            this.createOppositeOrder(marketId, ticket_type, price, remainingQuantity, userId, "SELL");
            this.placeOrderOnBook(marketId, ticket_type, price, remainingQuantity, userId,"SELL");
    ...
    private executeTrade(buyerId: string, sellerId: string, marketId: string, 
                        ticketType: "YES" | "NO", quantity: number, price: number) {
        ...
        console.log(`Trade executed: ${buyerId} bought ${quantity} ${ticketType} at ${price} from ${sellerId}`);
    }

    private createOppositeOrder(marketId: string, ticketType: "YES" | "NO", 
                               price: number, quantity: number, userId: string, 
        ...
        const targetBook = this.OrderBook[marketId][oppositeSide][oppositeOrderType];
        ...
        console.log(`Created opposite order: ${oppositeOrderType} ${oppositeSide} at ₹${oppositePrice} for ${quantity} shares`);

    }

  

    private placeOrderOnBook(marketId: string, ticketType: "YES" | "NO", 
                                price: number, quantity: number, userId: string, side:"BUY"|"SELL") {
    ...
    private initializeUserStock(userId: string, marketId: string, ticketType: "YES" | "NO") {
    ...
    createNewMarket(data: Extract<SubscribeMessageType, { type: "CREATE_MARKET" }>) {
        ...
        this.OrderBook[marketId] = {
            YES: {
                BUY: { totalQty: 100, priceLevels: { "4.5": { totalQty: 100, orders: [{ userId: "ADMIN", stock_quantity: 100 }] } } },
                SELL: { totalQty: 100, priceLevels: { "5": { totalQty: 100, orders: [{ userId: "ADMIN", stock_quantity: 100 }] } } }
            },
            NO: {
                BUY: { totalQty: 100, priceLevels: { "4.5": { totalQty: 100, orders: [{ userId: "ADMIN", stock_quantity: 100 }] } } },
                SELL: { totalQty: 100, priceLevels: { "5": { totalQty: 100, orders: [{ userId: "ADMIN", stock_quantity: 100 }] } } }
    ...
    createNewUser(data: Extract<SubscribeMessageType, { type: "CREATE_USER" }>) {
        ...
        this.User_Balances[userId] = { balance: 200000, locked: 0 }; // ₹2000 in paise
        this.Stock_Balances[userId] = {};
