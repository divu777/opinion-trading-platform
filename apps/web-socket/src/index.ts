...
const handleMarketUpdate = (
  response: Extract<MessageRecieved, { type: "MARKET_UPDATE" }>["payload"],
  ws: WebSocket
) => {
  try {
    const { orderBook, marketId } = response;

    let marketExist = GlobalMarkets[marketId];

    if (!marketExist) {
      GlobalMarkets[marketId] = [];
      console.log("first event for creating returned ");
      return;
    }

    marketExist.forEach((socket) => {
      socket.send(orderBook);
    });
